    ...
    const self = useRef({
        ...
        createWorkspace: async (showLoading: boolean) => {
            ...
            try {
                ...
                await services.createWorkspace(container, workspaceNameRef.current, allWorkspacePathsRef.current.find(item => item.value === selectedWorkspacePathRef.current)?.label ?? '');
